...
{
	if(opcode != 0xCB)
		printDebug("[Addr: %04X] [Opcode: %02X]   - %s\n", memAddr & 0xFFFF, opcode & 0xFF, instMap[opcode]);
}

void printCBInst(const int memAddr, const uint8_t opcode)
{
	printDebug("[Addr: %04X] [Opcode: CB%02X] - %s\n", memAddr & 0xFFFF, opcode & 0xFF, cbInstMap[opcode]); 
}
