...
void CartridgeHeader::getRomSize(data_t *rawRom, int *romSize, int *romBanks)
{
    // ROM size is stored in 0x148
    data_t romSizeCode = rawRom[0x148];
    int KB = 1024;
    int MB = 1024 * 1024;
    switch (romSizeCode)
    {
...
void CartridgeHeader::getRamSize(data_t *rawRom, int *ramSize, int *ramBanks)
{
    // RAM size stored in 0x149
    data_t ramSizeCode = rawRom[0x149];
    int KB = 1024;
    switch (ramSizeCode)
    {
        case 0x00:
...
std::string CartridgeHeader::getGameTitle(data_t *rawRom)
{
...
std::string CartridgeHeader::getLicenseCode(data_t* rawRom, data_t *oldLicense)
{
...
bool CartridgeHeader::isColorOnly(data_t *rawRom)
{
    return ((rawRom[0x0143] & 0x80) != 0);
}

bool CartridgeHeader::hasSGBFunctionality(data_t *rawRom)
{
    return (rawRom[0x0146] == 0x03);
}

bool CartridgeHeader::isJapanese(data_t *rawRom)
{
    return (rawRom[0x014A] == 0);
}

int CartridgeHeader::getMaskRomVersion(data_t *rawRom)
{
    return rawRom[0x14C];
}

bool CartridgeHeader::hasValidHeaderSum(data_t *rawRom)
{
    data_t sum = 0;
...
bool CartridgeHeader::hasValidGlobalSum(data_t *rawRom, int romSize)
{
...
std::string CartridgeHeader::getRomTypeString(data_t *rawRom)
{
